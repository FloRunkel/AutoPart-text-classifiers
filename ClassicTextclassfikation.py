...
class ClassicTextclassfikation(Klassifikator):
    ...
    def model_parameter(self):
        single_label_model_args = ClassificationArgs()
        single_label_model_args.labels_list = [0,1,2,3]
        single_label_model_args.overwrite_output_dir = True
        single_label_model_args.learning_rate = self.lrs
        single_label_model_args.num_train_epochs = self.epochs
        return single_label_model_args
    
    # Erzeugung eines Klassifikationsmodels für die jeweilige Label-Klassifikation  
    # @Return: Das Klassifikationsmodell mit den Klassificationsparameter

    def get_Classification_Model(self): 
        return ClassificationModel(self.model, self.modelname, args=self.model_parameter(), num_labels=4, use_cuda=False)
    ...
    def evaluate(self, model, test_data):
        ...
        mcc = matthews_corrcoef(target_list, prediction_list)
