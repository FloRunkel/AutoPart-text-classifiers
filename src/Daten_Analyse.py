...
class Daten_Analyse(Data):
    ...
    def verteilung_Data_plot(self):
        ...
        plt.bar(plot_labeled_texts.keys(), plot_labeled_texts.values())
        plt.show()
    ...
    def satzlaenge_Data_plot(self):
        ...
        satz_laengen_sonstiges = [len(value) for key, value in labeled_texts.items() if value == "Sonstiges"]
        ...
        satz_laengen_ecommerce = [len(value) for key, value in labeled_texts.items() if value == "Softwareentwicklung im E-Commerce"]
        ...
        satz_laengen_bank = [len(value) for key, value in labeled_texts.items() if value == "Softwareentwicklung im Bankensektor"]
        durchschnitt_bank = sum(satz_laengen_bank) / len(satz_laengen_bank)
        durchschnittsatzlaenge.append(durchschnitt_bank)

        satz_laengen_cloud = [len(value) for key, value in labeled_texts.items() if value == "Softwareentwicklung für Cloud-Lösungen"]
        durchschnitt_cloud = sum(satz_laengen_cloud) / len(satz_laengen_cloud)
        durchschnittsatzlaenge.append(durchschnitt_cloud)

        plt.xticks(rotation=90, fontsize=14)
        plt.title("Satzlänge nach Kategorie", fontsize=16, fontweight="normal")
        plt.ylabel('⌀  Satzlängez', fontsize=16, fontweight="normal")
    ...
    def woerterHaeufigkeit(self): 
        ...
        for key, value in labeled_texts.items():
            if value == "Sonstiges":
                woerter = key.split()
