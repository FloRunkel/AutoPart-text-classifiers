...
class FewShot(Klassifikator):
    ...
    def __init__(self, model, lrs, epochs, multilabel_selecter):
        self.model = model
        self.epochs = epochs 
        self.lrs = lrs
    ...
    def initial_TARS_model(self, corpus):
        ...
        tars.add_and_switch_to_new_task(task_name='Textclassification', label_dictionary=corpus.make_label_dictionary(label_type = 'Textclassification'), label_type = 'Textclassification', multi_label= self.multilabel_selecter)
        return tars
    ...
    def train(self, corpus):
        self.get_ModelTrainier(corpus).train(base_path='resources/taggers/Textclassification',learning_rate=self.lrs, mini_batch_size=1,max_epochs=self.epochs, train_with_dev=True, save_final_model=True, warmup_fraction=0.3)
    ...
    def evaluate(self, test_text, test_labels):
        ...
        if self.multilabel_selecter == False: 
            prediction_list=[]

            for text in test_text:
                sentence = Sentence(text)
                tars.predict(sentence, multi_label=self.multilabel_selecter)
            ...
            print("Bei einer MultiLabel Klassifikation müssen die Ausgabewerte angeschaut werden.")
            return 0,0,0
    ...
    def prediction(self, text):
        ...
        print(text + " - Predicted:", prediction_list)
